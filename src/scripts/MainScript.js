...
var Statistics = (function() {
	...
	var aminoAcidsReference = [
			new AminoAcid("Alanine", "Ala", "A", 89.09, "hydrophobic", [ "GCU",
					"GCC", "GCA", "GCG" ]),
			...
			new AminoAcid("Phenylalanine", "Phe", "F", 165.19, "hydrophobic", [
					"UUU", "UUC" ]),
			new AminoAcid("Glycine", "Gly", "G", 75.07, "hydrophobic", [ "GGU",
					"GGC", "GGA", "GGG" ]),
			new AminoAcid("Histidine", "His", "H", 155.16, "positive", [ "CAU",
					"CAC" ]),
			new AminoAcid("Isoleucine", "Ile", "I", 131.17, "hydrophobic", [
					"AUU", "AUC", "AUA" ]),
			new AminoAcid("Lysine", "Lys", "K", 146.19, "positive", [ "AAA",
					"AAG" ]),
			...
			new AminoAcid("Glutamine", "Gln", "Q", 146.15, "hydrophilic", [
					"GAA", "CAG" ]),
			new AminoAcid("Arginine", "Arg", "R", 174.20, "positive", [ "CGU",
					"CGC", "CGA", "CGG", "AGA", "AGG" ]),
			new AminoAcid("Serine", "Ser", "S", 105.09, "hydrophilic", [ "UCU",
					"UCC", "UCA", "UCG", "AGU", "AGC" ]),
			new AminoAcid("threonine", "Thr", "T", 119.12, "hydrophilic", [
					"ACU", "ACC", "ACA", "ACG" ]),
			new AminoAcid("Valine", "Val", "V", 117.15, "hydrophobic", [ "GUU",
					"GUC", "GUA", "GUG" ]),
	...
	function runWeights(sequence) {
		var aminoAcids = loadSequence(sequence), oneLetterLookup = createOneLetterLookup(aminoAcidsReference), fullLetterLookup = createFullLetterLookup(aminoAcidsReference), finalObj = getAllWeightPercentages2(
				aminoAcids, fullLetterLookup);
		return finalObj;
	}
	...
	function capCodon(codonSeq) {
		...
		if (newString.substring(length - 3) === "UAA"
				|| newString.substring(length - 3) === "UAG"
				|| newString.substring(length - 3) === "UGG") {
